//  BeatingHeartApplet.java  Copyright (c) Kari Laitinen

//  http://www.naturalprogramming.com

//  2007-11-27  File created.
//  2007-12-13  Latest modification.

/* This program demonstrates so-called active rendering.
   Active rendering means that a program actively decides when
   it wants to draw its drawings to its window. Active rendering
   is thus a kind of opposite to using the repaint() and paint()
   methods, the use of which can be described as "passive
   rendering".

   By using active rendering drawing operations are faster,
   which is necessary, for example, when game programs are
   written.

   This program draws a heart to the screen. The heart seems
   to beat as the scale in which it is draws is being waried.
   As the heart is represented by a GeneralPath object, this
   program also demonstrates the use of 2D graphics.
...
public class BeatingHeartApplet extends JApplet
                                       implements Runnable
{
